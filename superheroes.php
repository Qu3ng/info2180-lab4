...
if ($query) {
    // Search for superhero by alias or name
    $found = false;
    foreach ($superheroes as $superhero) {
        // Case-insensitive search in both alias and name
        if (stripos($superhero['alias'], $query) !== false || stripos($superhero['name'], $query) !== false) {
            // Return in format: Alias|Name|Biography
            echo $superhero['alias'] . '|' . $superhero['name'] . '|' . $superhero['biography'];
            $found = true;
            break;
        }
    }
    ...
    // No query - return all superheroes as list
    foreach ($superheroes as $superhero) {
